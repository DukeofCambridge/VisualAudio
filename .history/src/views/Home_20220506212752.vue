    ...
    <el-container>
     ...
     <el-footer class="palyer">
      ...
      <hr style="FILTER:alpha(opacity=100,finishopacity=0,style=1); margin-top:-16px;width:50%;margin-right:50%" width="97%" color=#00cc99 SIZE=3>
       ...
       style="margin: 0 12px;:style="{'font-size':item.choose?'12px':'14px','color':accolor}""
       v-bind:class="{ 'opacity' : is_play_visible }"
       @click="change"
       ></el-image>
       <el-image :src="require(`/src/assets/home/pause.svg`)"
       style="margin: 0px 0px 0px -56px; position:absolute"
       v-bind:class="{ 'opacity' : is_pause_visible }"
       @click="change"
       ></el-image>
  ...
  methods: {
    change() {
      ...
      if(this.is_stop == true) {
        ...
        this.is_stop = false;
        this.is_play_visible = 0
        this.is_pause_visible = 1
      }
      //启动 -> 暂停
      else {
        console.log("暂停")
        ...
        this.is_stop = true;
        this.is_play_visible = 1
        this.is_pause_visible = 0
      }
